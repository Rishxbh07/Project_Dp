...
const JoinPlanPage = ({ session }) => {
    ...
    useEffect(() => {
        const fetchAllDetails = async () => {
            ...
            if (!session?.user?.id) {
                // Wait briefly for session to load, if still missing, show error
                // For now, we assume if session is passed as prop, it's reliable or null
    ...
    // --- MAIN ACTION: Handle Payment & Joining ---
    const handleJoinPlan = async () => {
        ...
        try {
            ...
            const { data: orderData, error: orderError } = await supabase.functions.invoke('create-order', {
                body: { amount: priceDetails.total }
            });

            if (orderError) throw orderError;
            if (!orderData || !orderData.id) throw new Error("Failed to initialize payment.");
            ...
            const options = {
                ...
                handler: async function (response) {
                    try {
                        ...
                        console.error('Verification/Booking Error:', innerError);
                        setError(`Payment successful but joining failed: ${innerError.message}. Contact support.`);
            ...
            setError(`Could not start payment: ${error.message}`);
            setIsProcessingPayment(false);
        }
    };
    ...
    const finalizeBooking = async (gatewayTxId) => {
        ...
        if (!bookingData || !bookingData[0].success) throw new Error(bookingData[0].message || 'Booking creation failed.');
    ...
    if (error || !listing) return <p className="text-center text-red-500 mt-8 px-4">{error || 'Plan details could not be loaded.'}</p>;
            ...
            <footer className="fixed bottom-24 left-0 right-0 z-10">
                <div className="max-w-md mx-auto p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-t border-gray-200 dark:border-white/10 rounded-t-3xl shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)]">
                    <div className="flex justify-between items-center">
                        ...
                        >
                            {isProcessingPayment ? 'Processing...' : loading ? 'Loading...' : isHost ? "This is Your Plan" : isAlreadyJoined ? "Already Joined" : (seats_available > 0 ? 'Proceed to Pay' : 'Plan Full')}
                        </button>
                    </div>
                </div>
            </footer>
